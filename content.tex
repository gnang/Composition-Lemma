...
where
\[
\begin{aligned}R1_{f,g} & =\Bigg(\prod_{\substack{v\in f^{-1}(\{f(n-1)\})\\
f(n-1)<u<v
}
...
\]
By averaging, we obtain such a transposition $\tau$
also fixes
...
\]
as described in equation \eqref{eq:chosen-summand}. In fact, by Lemma \ref{lem:variable-dependency}, the summand of $R_{f,g}$ described in equation \eqref{eq:chosen-summand} is congruent to
\[
