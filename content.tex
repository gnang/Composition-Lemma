...
\]
matches the sign change resulting from the action of $\tau$ on the remainder of the first factor $F(\mathbf{x})$. Observe that
...
\]
In effect for each $\sigma\in\text{S}_{n}$, in equation \eqref{eq:ExpansionP} within the $j$-th summand we have replaced the Lagrange basis
\[
...
\]
This broadens symmetries among variables indexed by $\overline{S_{j}}$ which don't feature in the $j$-th summand $\underset{g\in(\mathbb{Z}_{D_{j}})^{S_{j}}}{\sum}c_{j,g}\,\underset{i\in S_{j}}{\prod}(x_{i})^{g(i)}$.
\[
...
\end{array}
 \]
 The last equality above features the indexing change of variable $u=\varphi^{-1}(i)$ and $v=\varphi^{-1}(j)$. Thus, if $\varphi \in \text{Aut}\left(G_f\right)$ then $P_{f}(x_{\varphi(0)},\ldots,x_{\varphi(n-1)})$ is up to sign equal to $P_{\varphi f\varphi^{-1}}$, in accordance with Definition \ref{defn:polynomial-grace-definition}. Furthermore, by the proof of Proposition \ref{prop:polynomial-grace-certificate}, the action of $\varphi$
