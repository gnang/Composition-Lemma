...
\end{equation}
we simply say $P \equiv Q$. All congruence relations in this paper are modulo the set immediately above.
\end{defn}
...
directed graph $G_f$ prescribed by $f$ is graceful if and only if $0\not\equiv \; P_f(\mathbf{x})$.
\end{proposition}
\begin{proof}
...
Consider an arbitrary contraction $f\in\mathbb{Z}_{n}^{\mathbb{Z}_{n}}$ i.e. subject to the fixed point condition
...
the claim holds for all functional digraph of diameter less than $3$. We now turn our attention
to functional trees of diameter greater or equal to $3$. We now proceed to show that
\[
\max_{\sigma\in\text{S}_{n}}\left|\left\{ \left|\sigma f^{(2)}\sigma^{-1}(i)-i\right|:i\in\mathbb{Z}_{n}\right\} \right|\le\max_{\sigma\in\text{S}_{n}}\left|\left\{ \left|\sigma f\sigma^{-1}(i)-i\right|:i\in\mathbb{Z}_{n}\right\} \right|\Longleftrightarrow P_{f^{(2)}}({\bf x})\not\equiv0\implies P_{f}({\bf x})\not\equiv0.
\]
By construction
\begin{equation}
n=\max_{\sigma\in\text{S}_{n}}\left|\left\{ \left|\sigma f\sigma^{-1}(i)-i\right|:i\in\mathbb{Z}_{n}\right\} \right|\Longleftrightarrow P_{f}({\bf x})\not\equiv0.
\end{equation}
The backward claim is the simplest of the two claims. We see
...
\end{equation}
This contradicts the assertion of the composition lemma, thereby
...
\end{equation}
We show that
\begin{equation} \label{eq:composition-lemma-general}
P_{g} ({\bf x}) \not\equiv 0  \implies  P_{f} ({\bf x}) \not\equiv 0.
\end{equation}
...
that all functional trees are graceful, which in turn implies that the \textit{Composition Lemma} as stated in Lemma \ref{lem:composition-lemma}  holds. Just as in the previous case, the assertion 
 \ref{eq:composition-lemma-general}.
...
\end{equation}
For if was the condition above was not met, we relabel the vertices of $G_f$ to ensure that this was the case. Note that such a relabeling does not affect the property we seek to prove. We prove by contradiction the contrapositive claim
\[
...
}\left({\color{red}b_{v}}+{\color{red}b_{u}}+2\,{\color{blue}a_{f(n-1)}}\right)\times\\[10pt]
 & \prod_{\substack{v\in f^{-1}\left(\{f(n-1)\}\right)\\
f\left(n-1\right)<u<v
}
}\left({\color{red}b_{v}}-{\color{red}b_{u}}+0\,{\color{blue}a_{f(n-1)}}\right),
\end{aligned}
...
the contrapositive of claim \eqref{eq:composition-lemma-generalized} is false, i.e., for some $f$ subject to conditions
described in our premise, we have
\begin{equation}
0\equiv{\color{red}{\color{red}P_{f}}}\;\text{ and }\;0\not\equiv P_{g}.
\end{equation}
Owing to equation \eqref{eq:Pg-equals-Pf-plus-Rfg}, we obtain
...
}\left(x_{u}-x_{v}\right)\bigg){\color{blue}(}x_{f^{\left(2\right)}\left(n-1\right)}{\color{blue}-x_{f\left(n-1\right)})}^{m}.   
\end{equation}
...
}\left({\color{red}b_{v}}-{\color{red}b_{u}}\right)\right)\,R1_{f,g}(\mathbf{x}),
\]
where
\[
\begin{aligned}R1_{f,g} & =\quad\Bigg(\prod_{\substack{v\in f^{-1}(\{f(n-1)\})\\
f(n-1)<u<v
}
...
\end{aligned}
\]
Observe that for each $\sigma\in\left\{ \theta\in\text{S}_{n}:G_{\theta g\theta^{-1}}\in\text{GrL}(G_{g})\right\}$,
...
\left(\sigma(u) - \sigma(v)\right).
\end{align}
More generally, by Proposition \ref{prop:multivariate-quotient-remainder}, the premise $P_{g}\equiv R_{f,g}$ implies 
\begin{equation}
P_g(h)=R_{f,g}\left(h\right)=v_{h}\cdot R1_{f,g}\left(h\right),\:\forall\:h\in\mathbb{Z}_{n}^{\mathbb{Z}_{n}}.
\end{equation}
By Lemma \ref{lem:transposition-invariance}, the same premise
...
fixes the remainder of $R_{f,g}$ given by
\begin{equation}
\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}v_{\sigma}\cdot R1_{f,g}(\sigma)\prod_{\substack{i\in\mathbb{Z}_{n}\\
j_{i}\in\mathbb{Z}_{n}\setminus\{\sigma(i)\}
}
}\bigg(\frac{x_{i}-j_{i}}{\sigma(i)-j_{i}}\bigg),
\end{equation}
which in turn by averaging implies that such a transposition $\tau$
also fixes
\begin{equation}\label{eq:averaging}
\bigg(\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}v_{\sigma}\bigg)\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}R1_{f,g}(\sigma)\prod_{\substack{i\in\mathbb{Z}_{n}\\
j_{i}\in\mathbb{Z}_{n}\setminus\{\sigma(i)\}
}
}\bigg(\frac{x_{i}-j_{i}}{\sigma(i)-j_{i}}\bigg).
\end{equation}
As a consequence of the multi-binomial expansion, the sum expressing
$R1_{f,g}$ features as one of its summand a unique monochromatic blue binomial summand given by
\[
\left({\color{blue}a_{f\left(n-1\right)}}\right)^{m}=c\,{\color{blue}(}x_{f^{\left(2\right)}\left(n-1\right)}{\color{blue}-x_{f\left(n-1\right)})}^{m}.
\] 
This monochromatic summand makes up the blue part of the chosen bichromatic summand of $R_{f,g}$ given by
\[
c\,\underset{0\le i<j<n}{\prod}\left(x_{j}-x_{i}\right)\prod_{0\le u<v\le f\left(n-1\right)}\left((x_{f\left(v\right)}-x_{v})^{2}-(x_{f\left(u\right)}-x_{u})^{2}\right)\bigg(\prod_{\substack{v\in f^{-1}\left(\left\{ f\left(n-1\right)\right\} \right)\\
f\left(n-1\right)<u<v
}
...
\]
which is also described by equation \eqref{eq:chosen-summand}. In fact the summand of $R_{f,g}$ described in equation \eqref{eq:chosen-summand} is congruent to
\[
c\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}v_{\sigma}\cdot {\color{blue}\big(}\sigma f^{(2)}(n-1){\color{blue}-\sigma f(n-1)\big)}^{m}\prod_{\substack{i\in\mathbb{Z}_{n}\\
j_{i}\in\mathbb{Z}_{n}\setminus\{\sigma(i)\}
}
}\bigg(\frac{x_{i}-j_{i}}{\sigma(i)-j_{i}}\bigg).
\]
As prescribed by equation \eqref{eq:averaging}, we apply the averaging argument to each bichromatic summand of $R_{f,g}$. In particular, applying the averaging argument to the chosen summand yields the polynomial
\[
c\,\bigg(\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}v_{\sigma}\cdot{\color{blue}\big(}\sigma f^{(2)}(n-1){\color{blue}-\sigma f(n-1)\big)}^{m}\bigg)\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}\prod_{\substack{i\in\mathbb{Z}_{n}\\
j_{i}\in\mathbb{Z}_{n}\setminus\{\sigma(i)\}
}
}\bigg(\frac{x_{i}-j_{i}}{\sigma(i)-j_{i}}\bigg).
\]
By Lemma \ref{lem:variable-dependency}, the remainder of the polynomial immediately above is equal to
\[
c\,\bigg(\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}v_{\sigma}\cdot{\color{blue}\big(}\sigma f^{(2)}(n-1){\color{blue}-\sigma f(n-1)\big)}^{m}\bigg)\sum_{\begin{array}{c}
\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
\end{array}}{\color{blue}\big(}\sigma f^{(2)}(n-1){\color{blue}-\sigma f(n-1)\big)}^{m}\times
\]
\[
...
a particular summand denoted as $P$.
\begin{enumerate}
\item Option 1: The candidate transposition of a pair of variables fixes
the chosen summand $P$. This occurs when $P$ is symmetric in the
chosen pair of variables being transposed.
\item Option 2: The candidate transposition of the chosen pair of variables
does not fix $P$ (i.e., Option 1 does not apply) but induces in turn
a transposition which exchanges the chosen summand $P$ with some
other summand from the partition say $Q$. This occurs, for instance,
if we consider the sum $P+Q$ where $P=(x_{0})^{2}\,x_{1}$ and $Q=x_{0}\,(x_{1})^{2}$.
In this example, we see that transposition which exchanges variables
$x_{0}$ with $x_{1}$ does not fix $P$, but it induces a transposition
which exchanges the summand $P$ with the summand $Q$.
\item Option 3: The candidate transposition of a pair of variables neither
fixes $P$ nor does it induce a transposition which exchanges $P$
with some other summand (i.e., neither Option 1 nor Option 2 applies).
Instead, $P$ is such that a symmetry broadening cancellation occurs.
Such a cancellation must involve interaction between the non-vanishing
monomials within the monomial support of $P$ with the non-vanishing monomials
within the support of other summands. Option 3 occurs, for instance,
if we take $P=-x_{1}$ and $Q=x_{0}+2x_{1}$. We see that in this
...
However $P+Q=x_{0}+x_{1}$ is symmetric and thus admits the said transposition
...
of the chosen summand does not exchange with the remainder of any
...
bi-chromatic summand in $R1_{f,g}$ depends on $3$ or more variables.
...
canonical representative of bi-chromatic summands in $R1_{f,g}$ of
the form
\[
...
\[
\tau\notin\text{Aut}\left(\overline{P}_{g}\right).
\]
