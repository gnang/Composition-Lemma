...
\end{proof}
For notational convenience, we denote by $L_{g}(\mathbf{x})$ the Lagrange basis polynomial associated with $g\in\mathbb{Z}_{n}^{\mathbb{Z}_{n}}$  
\[
...
\begin{proposition}
 ...
 Let $f,g \in\mathbb{Z}_{n}^{\mathbb{Z}_{n}}$. For congruence classes modulo the ideal generated by $\{d(x_{i}):i \in \Z_n\}$, we have
\[
...
\]
We now show that $\big(L_{f}(\mathbf{x})\big)^{2}-L_{f}(\mathbf{x})\equiv 0$
\begin{align*}
...
and $\mathscr{S}_{u}\subseteq\mathbb{Z}_{n}$ for all $u\in\mathbb{Z}_{m}$. For positive integers $\{D_{u}:u\in\mathbb{Z}_{m}\}$ let
\begin{equation}
...
\end{equation}
such that  
 for some integer $\kappa$ we have $F(\boldsymbol{\alpha}_{f\tau^{-1}})=(-1)^{\kappa} F(\boldsymbol{\alpha}_{f})$  
 for all $f\in \Z_{n}^{\Z_{n}}$,
 where $\tau$ denotes a transposition  which exchanges a fixed pair of variables. If  $F(\boldsymbol{\alpha}_{f})$ vanishes for all $f\in \mathbb{Z}_{n}^{\mathbb{Z}_{n}}\backslash\text{S}_{n}$, then
\[
...
\[
\overline{P}\equiv\overline{F}\sum_{j\in\mathbb{Z}_{m}}\big(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\,\prod_{i\in\mathscr{S}_{j}}(x_{i})^{g(i)}\big)
\]
\[
\implies\overline{P}\equiv\big(\sum_{\sigma\in\text{S}_{n}}F(\boldsymbol{\alpha}_{\sigma})\,L_{\sigma}(\mathbf{x})\big)\sum_{j\in\mathbb{Z}_{m}}\big(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\,\prod_{i\in\mathscr{S}_{j}}(x_{i})^{g(i)}\big)
\]
\[
\equiv\big(\sum_{\sigma\in\text{S}_{n}}F(\boldsymbol{\alpha}_{\sigma})\,L_{\sigma}(\mathbf{x})\big)\sum_{j\in\mathbb{Z}_{m}}\sum_{h_{j}\in\mathbb{Z}_{n}^{\mathscr{S}_{j}}}\bigg(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\prod_{i\in\mathscr{S}_{j}}\big(\alpha_{h_{j}(i)}\big)^{g(i)}\bigg)\,L_{h_{j}}(\mathbf{x}|_{\mathscr{S}_{j}})
\]
\[
\implies\big(\sum_{\sigma\in\text{S}_{n}}F(\boldsymbol{\alpha}_{\sigma})\,L_{\sigma}(\mathbf{x})\big)\sum_{j\in\mathbb{Z}_{m}}\sum_{h_{j}\in\mathbb{Z}_{n}^{\mathscr{S}_{j}}}\bigg(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\prod_{i\in\mathscr{S}_{j}}\big(\alpha_{h_{j}(i)}\big)^{g(i)}\bigg)\,L_{h_{j}}(\mathbf{x}|_{\mathscr{S}_{j}})=
\]
\[
\sum_{j\in\mathbb{Z}_{m}}\sum_{(\sigma,h_{j})\in\text{S}_{n}\times\mathbb{Z}_{n}^{\mathscr{S}_{j}}}\bigg(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\,\prod_{i\in\mathscr{S}_{j}}\big(\alpha_{h_{j}(i)}\big)^{g(i)}\bigg)F(\boldsymbol{\alpha}_{\sigma})\,L_{h_{j}}(\mathbf{x}|_{\mathscr{S}_{j}})L_{\sigma}(\mathbf{x}|_{\mathscr{S}_{j}})L_{\sigma}(\mathbf{x}|_{\overline{\mathscr{S}_{j}}}),
\]
where $\overline{\mathscr{S}_{j}}:=\mathbb{Z}_n\setminus \mathscr{S}_{j}$. Recall that $L_{\sigma}(\mathbf{x})$ has a trivial automorphism group for all $\sigma \in \text{S}_n$. Each summamnd in the sum above is either such that
\[
\bigg(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\,\prod_{i\in\mathscr{S}_{j}}\big(\alpha_{\sigma(i)}\big)^{g(i)}\bigg)F(\boldsymbol{\alpha}_{\sigma})L_{h_{j}}(\mathbf{x}|_{\mathscr{S}_{j}})L_{\sigma}(\mathbf{x}|_{\mathscr{S}_{j}})L_{\sigma}(\mathbf{x}|_{\overline{\mathscr{S}_{j}}})\equiv0
\]
in which case the said summand imposes no constraints at all on the membership of any permutation into the automorphism group of $\overline{P}$. Thus such summands play no role in ascertaining Aut$(\overline{P})$.  Alternatively a summand
\[
\bigg(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\,\prod_{i\in\mathscr{S}_{j}}\big(\alpha_{\sigma(i)}\big)^{g(i)}\bigg)F(\boldsymbol{\alpha}_{\sigma})L_{h_{j}}(\mathbf{x}|_{\mathscr{S}_{j}})L_{\sigma}(\mathbf{x}|_{\mathscr{S}_{j}})L_{\sigma}(\mathbf{x}|_{\overline{\mathscr{S}_{j}}})\not\equiv0
\]
is a member of an orbit
\[
...
\end{array}\right\} 
\]
which impose constraints on membership of permutations into  the automorphism group of $\overline{P}$. Therefore the remainder of such summand must be accounted for in order to ascertain Aut$(\overline{P})$. If $\tau$ lies in the automorphism group of $\overline{P}$ then there are two cases to consider for each variable indexing set $\mathscr{S}_{j}$. In the first case both variables interchanged by the transposition $\tau$ either lie in $\mathscr{S}_{j}$ or both lie in its complement  $\overline{\mathscr{S}_{j}}$. In this setting the orbit
\[
...
\end{array}\right\} 
\]
is up to signed fixed by the transposition $\tau$. Otherwise in the setting where only one of the variables exchanged by $\tau$ lies in $\mathscr{S}_{j}$, $\tau$ induces a transposition of the orbit of summands
\[
\left\{ \bigg(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\,\prod_{i\in\mathscr{S}_{j}}\big(\alpha_{\sigma(i)}\big)^{g(i)}\bigg)\,F(\boldsymbol{\alpha}_{\sigma})\,L_{\sigma}(\mathbf{x}):\begin{array}{c}
\sigma\in\text{S}_{n}\\
\text{s.t.}\\
...
\end{array}\right\} 
\]
with some other orbit of summands
\[
...
\end{array}\right\} 
\]
for we see that if that $F(\boldsymbol{\alpha}_{f\tau^{-1}})=- F(\boldsymbol{\alpha}_{f})$ for all $f\in \Z_{n}^{\Z_{n}}$ or alternatively $F(\boldsymbol{\alpha}_{f\tau^{-1}})= F(\boldsymbol{\alpha}_{f})$ for all $f\in \Z_{n}^{\Z_{n}}$, then the orbit summands
\[
\left\{ \bigg(\sum_{g\in(1+\mathbb{Z}_{D_{j}})^{\mathscr{S}_{j}}}c_{j,g}\,\prod_{i\in\mathscr{S}_{j}}\big(\alpha_{\sigma(i)}\big)^{g(i)}\bigg)\,F(\boldsymbol{\alpha}_{\sigma})\,L_{\sigma}(\mathbf{x}):\begin{array}{c}
\sigma\in\text{S}_{n}\\
\text{s.t.}\\
...
\end{array}\right\} 
\]
is not up to sign fixed by $\tau$. Thus pairing by this interchange $\mathscr{S}_{j}$ with $\mathscr{S}_{k}$ and vice versa.  

{\color{red}
I do not see what was missing and is fixed here in the argument by requiring each $S_j$ to be distinct as we have done, by requiring degree to be greater than 0 for each variable.
}

In other words $\tau(\mathscr{S}_{j})=\mathscr{S}_{k}$.
Observe that
...
\]
These sums over Lagrange bases are as block subject to the same pairing argument described above. Thus $\tau$ also fixes the symmetrized polynomial. By properties of Lagrange basis polynomials it follows that the symmetrized polynomial equals
\[
