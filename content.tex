...
all claima hold for all functional digraph of diameter less than $3$. We now turn our attention
...
The exact same reasoning establishes (ii) $\iff$ (iii), for we have
\begin{equation}
...
\end{equation}
For if the condition above was not met, we relabel the vertices of $G_f$ to ensure that this was the case. Note that such a relabeling does not affect the property we seek to prove. We prove by contradiction the contrapositive claim
\[
...
fixes the remainder of $R_{f,g}$ is congruent to
given by
\begin{equation}
...
\[
\left(\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}v_{\sigma}\prod_{\substack{i\in\mathbb{Z}_{n}\\
j_{i}\in\mathbb{Z}_{n}\setminus\{\sigma(i)\}
}
}\bigg(\frac{x_{i}-j_{i}}{\sigma(i)-j_{i}}\bigg)\right)\left(\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}R1_{f,g}(\sigma)\prod_{\substack{i\in\mathbb{Z}_{n}\\
j_{i}\in\mathbb{Z}_{n}\setminus\{\sigma(i)\}
}
}\bigg(\frac{x_{i}-j_{i}}{\sigma(i)-j_{i}}\bigg)\right)
\]
and in turn by averaging implies that such a transposition $\tau$
also fixes
...
\]
which is also described by equation \eqref{eq:chosen-summand}. In fact, by Lemma \ref{lem:variable-dependency}, the summand of $R_{f,g}$ described in equation \eqref{eq:chosen-summand} is congruent to
\[
c\left(\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}v_{\sigma}\prod_{\substack{i\in\mathbb{Z}_{n}\\
j_{i}\in\mathbb{Z}_{n}\setminus\{\sigma(i)\}
}
}\bigg(\frac{x_{i}-j_{i}}{\sigma(i)-j_{i}}\bigg)\right)\sum_{\substack{\sigma\in\text{S}_{n}\\
G_{\sigma g\sigma^{-1}}\in\text{GrL}(G_{g})
}
}{\color{blue}\big(}\sigma f^{(2)}(n-1){\color{blue}-\sigma f(n-1)\big)}^{m}\times
\]
\[
